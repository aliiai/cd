@forelse($campaigns as $campaign)
    <tr class="hover:bg-gray-50 transition-colors duration-150">
        ...
        <td class="px-6 py-4 whitespace-nowrap">
            ...
            <div class="text-xs text-gray-500">{{ Str::limit($campaign->message ?? '', 50) }}</div>
        </td>

        <!-- Provider -->
        <td class="px-6 py-4 whitespace-nowrap">
            <div class="text-sm text-gray-900">{{ $campaign->owner->name ?? 'غير محدد' }}</div>
            <div class="text-xs text-gray-500">{{ $campaign->owner->email ?? '' }}</div>
        </td>
