...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-primary-50 to-secondary-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 py-12">
    <div class="w-full mx-auto sm:px-6 lg:px-8 max-w-2xl">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-4xl font-bold text-gray-900 dark:text-gray-100 mb-2">تعديل الصلاحية: {{ $permission->name }}</h1>
            <p class="text-lg text-gray-600 dark:text-gray-400">تعديل بيانات الصلاحية</p>
        </div>
                    ...
                    @if($permission->roles->count() > 0)
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                                الأدوار المستخدمة لهذه الصلاحية
                            </label>
                            <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 border border-gray-200 dark:border-gray-600">
                                <div class="flex flex-wrap gap-2">
                                    @foreach($permission->roles as $role)
                                        <span class="px-3 py-1 bg-primary-100 dark:bg-primary-900/30 text-primary-800 dark:text-primary-300 rounded-full text-sm font-semibold border border-primary-200 dark:border-primary-800">
                                            {{ $role->name }}
                                        </span>
                                    @endforeach
                                </div>
