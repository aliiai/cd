...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('sms_logs', function (Blueprint $table) {
            $table->id();
            $table->string('event_type')->default('manual'); // vendor_registered, order_created, campaign, etc.
            $table->string('entity_type')->nullable(); // App\Models\Order, App\Models\CollectionCampaign, etc.
            ...
            $table->text('provider_response')->nullable(); // Full provider response (JSON)
            $table->text('error_message')->nullable(); // Error details if failed
            $table->integer('attempt')->default(1); // Attempt number
            $table->string('hash')->unique()->nullable(); // Unique hash for idempotency
